...
public:
    ...
    void setDebugOutput(bool debug, wm_debuglevel_t level); // log line prefix, default "*wm:"
    ...
    void setCaptivePortalEnable(bool enabled);
    ...
    void setWebPortalClientCheck(bool enabled);
    ...
    void setScanDispPerc(bool enabled);
    ...
    void setDisableConfigPortal(bool enable);
...
protected:
    ...
    bool _showPassword = false;           // show or hide saved password on wifi form, might be a security issue!
    bool _shouldBreakAfterConfig = false; // stop configportal on save failure
    bool _configPortalIsBlocking = true;  // configportal enters blocking loop
    bool _enableCaptivePortal = true;     // enable captive portal redirection
    bool _wifiAutoReconnect = true;       // there is no platform getter for this, we must assume its true and make it so
    bool _apClientCheck = false;          // keep cp alive if ap have station
    bool _webClientCheck = true;          // keep cp alive if web have client
    bool _scanDispOptions = false;        // show percentage in scans not icons
    bool _paramsInWifi = true;            // show custom parameters on wifi page
    bool _showInfoErase = true;           // info page erase button
    bool _showInfoUpdate = true;          // info page update button
    ...
    bool _disableConfigPortal = true;     // FOR autoconnect - stop config portal if cp wifi save
...
public:
    bool _preloadwifiscan = false;    // preload wifiscan if true
    unsigned int _scancachetime = 30000; // ms cache time for preload scans
    bool _asyncScan = false;          // perform wifi network scan async
...
protected:
    ...
    bool _tryWPS = false; // try WPS on save failure, unsupported
    void startWPS();
#endif
...
#warning "ESP32 set country unavailable"
    ...
    bool portalTimeoutResult = false;

    bool portalAbortResult = false;
