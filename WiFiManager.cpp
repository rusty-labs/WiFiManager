 ...
 * @access public
 */
int WiFiManager::getParametersCount() const
{
  return _paramsCount;
}
...
void WiFiManager::loadCredentials()
{
...

  {
    ...
    _ssid = String(reinterpret_cast<char *>(tmp));
  }

  {
    ...
    _pass = String(reinterpret_cast<char *>(tmp));
  }
  ...
  if (esp_wifi_get_config(WIFI_IF_STA, &conf) == ESP_OK)
  {
    ...
    _pass = String(reinterpret_cast<const char *>(conf.sta.password));
    DEBUG_WM(WM_DEBUG_DEV, F("Loaded SSID: "), _ssid);
    DEBUG_WM(WM_DEBUG_DEV, F("Loaded PASSWORD: "), _pass);
  }
#endif
}
...
bool WiFiManager::configPortalHasTimeout()
{
...
// init configportal
  ...
  while (1)
  {
    DEBUG_WM(WM_DEBUG_VERBOSE, "ESP.getFreeHeap(): ");
    DEBUG_WM(WM_DEBUG_VERBOSE, ESP.getFreeHeap());
...
bool WiFiManager::wifiConnectNew(String ssid, String pass, bool connect)
{
  ...
  // Here we just save wifi credentials without invoking a connection
  WiFi.persistent(true);
  ret = WiFi.begin(ssid.c_str(), pass.c_str(), 0, NULL, false);

  if (connect)
  {
    // Trying to connect without saving credentials
    WiFi.persistent(false);
    ret = ret && WiFi.begin(ssid.c_str(), pass.c_str(), 0, NULL, true);
    WiFi.persistent(true);
  }
...
bool WiFiManager::wifiConnectDefault()
{
  ...
  DEBUG_WM(WM_DEBUG_DEV, F("Using Password:"), WiFi_psk());
#endif
...
// DEBUG_WM(WM_DEBUG_DEV,"refresh flag:",server->hasArg(F("refresh")));
  ...
  if (WiFi_psk() != "")
  {
    pitem.replace(FPSTR(T_p), _showPassword ? WiFi_psk() : FPSTR(S_passph));
  }
  else
  {
...
void WiFiManager::handleWifiSave()
{
  ...
  // SAVE/connect here
  _ssid = server->arg(F("s"));
  const String newPass = server->arg(F("p"));

  if (newPass != String(S_passph))
  {
    _pass = newPass;
  }
...
//   handleErase(false);
// }
void WiFiManager::handleErase(bool opt)
{
#ifdef WM_DEBUG_LEVEL
 ...
 */
bool WiFiManager::captivePortal()
{
...
void WiFiManager::setDebugOutput(bool debug, wm_debuglevel_t level)
{
  _debugLevel = level;
  // _debugPrefix = prefix;
  setDebugOutput(debug);
 ...
 * @access public
 * @param {[type]} bool shouldBreak [description]
 */
 ...
 * @param bool alwaysShow [false]
 */
void WiFiManager::setShowPassword(bool show)
{
  _showPassword = show;
}
 ...
 */
void WiFiManager::setCaptivePortalEnable(bool enabled)
{
 ...
 * @param bool enabled [true]
 */
void WiFiManager::setWiFiAutoReconnect(bool enabled)
{
 ...
 */
void WiFiManager::setAPClientCheck(bool enabled)
{
 ...
 * @param bool enabled [true]
 */
void WiFiManager::setWebPortalClientCheck(bool enabled)
{
 ...
 * @param bool enabled [false]
 */
void WiFiManager::setScanDispPerc(bool enabled)
{
 ...
 * @param bool enabled [true]
 */
void WiFiManager::setEnableConfigPortal(bool enable)
{
 ...
 * @param bool enabled [true]
 */
void WiFiManager::setDisableConfigPortal(bool enable)
{
 ...
 * @param bool enabled
 */
void WiFiManager::setShowInfoErase(bool enabled)
{
 ...
 * @param bool enabled
 */
void WiFiManager::setShowInfoUpdate(bool enabled)
{
...
String WiFiManager::WiFi_psk() const
{
#if defined(ESP32)
